...
public class PanelMaze extends JPanel {
	...
	private GridCellContent[][] fillGridCells(In inputContents) {
		...
		GridCellContent[][] localCellContents = new GridCellContent[lines.length][firstLineList.length];
		int lineCounter = 0;
		...
		for (String currentLine : lines) {
			...
			for (int i = 0; i < currentLineList.length; i++) {
				localCellContents[lineCounter][cellCounter] = new GridCellContent(currentLineList[i]);
				cellCounter++;
			}
			lineCounter++;
		}
	...
	private void createPanelsFromGridCells() {
		...
		allPanels = new JPanel[allCellContents.length][allCellContents[0].length]; // makes identical rows/col #s as
																					// allCells [][]
		setLayout(new GridLayout(allCellContents.length, allCellContents[0].length)); // makes identical rows/col #s as
																						// allCells [][]
	...
	/**
	 * Updates this maze's components to display up-to-date information.
	 */
				...
				if (allPanels[i][j].getComponent(0) instanceof JButton) {
					...
					try {
						...
						if (!thisCellsContent.getCellText().equals(gm.player.nodesVisited.peek())) {
							// if player has not visited this node, and it's adjacent to the last node
							// visited,
